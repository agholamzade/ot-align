...
def extract_llm_features(llm_model_name, dataset, args):
    ...
    save_path = utils.to_feature_filename(
        args.output_dir, args.dataset, llm_model_name,
    ...
    language_model = load_llm(llm_model_name, qlora=args.qlora)
    ...
    save_dict = {
    "feats": torch.cat(llm_feats).cpu().to(torch.float32).numpy(),
    "num_params": llm_param_count,
    "mask": tokens["attention_mask"].cpu().to(torch.float32).numpy()}
...
def extract_lvm_features(lvm_model_name, dataset, args):
    ...
    save_path = utils.to_feature_filename(
        args.output_dir, args.dataset, lvm_model_name,
        pool=pool, prompt=None,
    )
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    ...
    transform_config = resolve_data_config(vision_model.pretrained_cfg, model=vision_model, verbose=True)
    for n, v in transform_config.items():
        print('\t%s: %s' % (n, str(v)))
    ...
    for i in trange(0, len(dataset), args.batch_size):
        with torch.no_grad():
            end_index = min(i + args.batch_size, len(dataset))
            ims = torch.stack([transform(dataset[j]['image']) for j in range(i, end_index)]).cuda()
    ...
    np.savez(save_path, feats= torch.cat(lvm_feats), num_params=lvm_param_count)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir",     type=str, default="/ptmp/agholamzadeh/GCC/features")
    ...
    llm_model_name = "openlm-research/open_llama_7b"
    lvm_model_name = "vit_base_patch16_224.mae"
    datasets.config.IN_MEMORY_MAX_SIZE = 40 * 1024 ** 3  # 40 GB in bytes
    # load dataset once outside    
    dataset = load_from_disk("/ptmp/agholamzadeh/GCC/ccd_train_100K.hf")

    extract_llm_features(llm_model_name, dataset, args)
    args.batch_size = 128
    
    extract_lvm_features(lvm_model_name, dataset, args)